...
class SellerUpdate(BaseModel):
    name: Optional[str] = Field(..., min_length=4)
    website: Optional[AnyUrl]
    email: Optional[EmailStr]  # Here we want to validate the email field only for that we can use field validator
...
class DimensionUpdate(BaseModel):
    ...
    width: Annotated[Optional[float], Field(gt=0, strict=True, description="Length in cm")] = None
    hieght: Annotated[Optional[float], Field(gt=0, strict=True, description="Length in cm")] = None
